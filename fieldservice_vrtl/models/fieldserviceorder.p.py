...
class FieldServiceOrder(models.Model):
    ...
    order_number = fields.Char(string="Reference Number", default=lambda self: _('New'), readonly=True, copy=False,
                               required=True)
    ...
    ## if VERSION <= "17.0"
    line_count_avg = fields.Integer(group_operator='avg', string='Occasion Count (AVG)', compute='_compute_line_count',
                                    store=True)
    total_duration = fields.Float(string='Total Duration', compute='_compute_total_duration', store=True,
                                  help='Total duration of all lines', group_operator=False)
    ## else
    ...
    def create_occasion(self):
        record = self.env['fieldservice.order.line'].create(
            [{'order_id': self.id, 'date_start': self.planned_start_datetime}])
    ...
    def create(self, vals_list):
        default_project_id = int(
            self.env['ir.config_parameter'].sudo().get_param('fieldservice.default_project_id') or 0)
    ...
    def _compute_reporter(self):
        for record in self:
            stakeholder_ids = list(
                filter(lambda stakeholder_id: stakeholder_id.partner_status == "reporter", record.stakeholder_ids))
    ...
    def create_occasion(self):
        record = self.env['fieldservice.order.line'].create(
            [{'order_id': self.id, 'date_start': self.planned_start_datetime, 'date_end': self.planned_start_datetime}])
        return record.open_planning_view()
